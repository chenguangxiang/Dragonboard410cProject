...
static DEVICE_ATTR(enable,S_IRUGO|S_IWUGO,hs100_show_enable,hs100_store_enable);
static DEVICE_ATTR(value,S_IRUGO|S_IWUGO,hs100_show_value,NULL);
static int us100_probe(struct platform_device *pdev){
	...
	if(gpio_is_valid(data->cmd_gpio)){
		result = gpio_request(data->cmd_gpio,"hs100_cmd_gpio");
		if(result<0){
	...
	if(gpio_is_valid(data->echo_gpio)){
		result = gpio_request(data->echo_gpio,"hs100_irq_gpio");
		if(result<0){
			pr_err("uanble to request irq gpio\n");
			goto err_parse_dt;
		}
		result = gpio_direction_input(data->echo_gpio);
