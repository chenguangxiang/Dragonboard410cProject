		...
		char *buf)
{
	...
	return sprintf(buf, "%u", pwm->duty_cycle);
}
		...
		char *buf)
{
	...
	return sprintf(buf, "%s", pwm->polarity ? "inversed" : "normal");
}
...
static DEVICE_ATTR(enable, 0666, pwm_enable_show, pwm_enable_store);
		...
		char *buf)
{
	...
	return sprintf(buf, "%u", chip->npwm);
}

static struct device_attribute pwm_chip_attrs[] = {
	__ATTR(export, 0222, NULL, pwm_export_store),
	__ATTR(unexport, 0222, NULL, pwm_unexport_store),
